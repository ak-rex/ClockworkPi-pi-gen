...
rm -f "${ROOTFS_DIR}/etc/vnc/updateid"
	...
	echo -n "Configuring Wayland Screen Rotation: "
		...
		sed -i '5 a binding_volume_up=KEY_VOLUMEUP' "${ROOTFS_DIR}/etc/wayfire/template.ini"
		sed -i '6 a command_volume_up=wfpanelctl volumepulse volu && ~/.config/volume_osd.sh' "${ROOTFS_DIR}/etc/wayfire/template.ini"
		sed -i '7 a binding_volume_down=KEY_VOLUMEDOWN' "${ROOTFS_DIR}/etc/wayfire/template.ini"
		sed -i '8 a command_volume_down=wfpanelctl volumepulse vold && ~/.config/volume_osd.sh' "${ROOTFS_DIR}/etc/wayfire/template.ini"
		echo "Done"
	else
			echo "Skipped"
	fi
